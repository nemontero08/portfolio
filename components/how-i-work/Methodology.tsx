 ...
 * NOT reinterpreted. Two things worth flagging:
 *
 ...
 * - Card 04 (DELIVERY/Launch)'s description is IDENTICAL, word-for-word, to
 *   the Principles section's own intro sentence ("My work follows a set of
 *   rules that ensure the final result is not just beautiful, but
 *   functional and scalable."). That looks like a copy-paste bug in the
 *   original site itself (same pattern as the testimonials' avatar alt-text
 *   bug found earlier) rather than intentional copy — reproduced verbatim
 *   regardless, per instructions not to fabricate or "fix" source content.
 *
...
const STEPS: { eyebrow: string; title: string; description: string; paths: IconPath[] }[] = [
  {
    eyebrow: "01 / STRATEGY",
    title: "Map",
    description: "Understand the problem, the business goals, and the real needs of the end user.",
    paths: [
      {
  ...
  },
  {
    ...
    title: "Flows",
    description: "Define navigation and touchpoints to ensure minimal friction.",
    paths: [
      {
  ...
  {
    eyebrow: "03 / PROTOTYPING",
    title: "Wireframes",
    description: "Visual structure without aesthetic distractions to validate information hierarchy.",
    paths: [
      {
  ...
  },
  {
    ...
    title: "Launch",
    description:
      "My work follows a set of rules that ensure the final result is not just beautiful, but functional and scalable.",
    paths: [
      {
...
function StepIcon({ paths }: { paths: IconPath[] }) {
  return (
  ...
  return (
    <>
      <div className={styles.intro}>
        <p className={styles.eyebrow}>METHODOLOGY &amp; FLOW</p>
        <div className={styles.introBody}>
          <h3 className={styles.heading}>I turn complex processes into digital products</h3>
          <p className={styles.paragraph}>
            I design as a decision system, not a screen production line. Before opening Figma, I define the
            problem, the flows, and the decisions that actually impact the business.
          </p>
        </div>
      </div>
