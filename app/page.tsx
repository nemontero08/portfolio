 ...
 * internals, which are untouched here). 12 columns x 12 rows, auto-placed
 ...
 * - Vitalmed (2/3, 8 cols, 4 rows — equalized to match the bottom content
 *   row's height) + a right column (1/3, 4 cols, one grid cell) that
 *   internally splits into About me (60%) over How I Work teaser (40%) via
 *   flex-grow (see VitalmedRightColumn) — 60/40 isn't representable as
 *   whole grid-row units, so it's not done with row-spans.
 * - Basalto + Lumine Gas (4 cols each = 8 total, matching the 2/3 column)
 *   + Testimonials (1/3, 4 cols), 4 rows — unchanged, this is the row the
 *   Vitalmed row was equalized to match.
 *
